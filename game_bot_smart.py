...
class GameBot:
    ...
    def find_template(
        ...
        # ===== 2. 截图 =====
        img = screenshot if screenshot is not None else self.take_screenshot()
        if img is None:
            return None
    ...
    def find_all_templates(
            ...
            min_template_saturation_ratio=None,
            screenshot=None,
            roi=None,
    ):
        if not self.running:
        ...
        color_mask = None
        template_median_saturation = None
        if (
            min_color_saturation is not None
            or min_template_saturation_ratio is not None
        ):
            mask_key = ('color-mask', template_name)
            median_key = ('median-saturation', template_name)
            if (
                mask_key not in self._template_cache
                or median_key not in self._template_cache
            ):
                template_hsv = cv2.cvtColor(template_color, cv2.COLOR_BGR2HSV)
                color_mask = template_hsv[:, :, 1] >= 80
                self._template_cache[mask_key] = color_mask
                self._template_cache[median_key] = float(np.median(
                    template_hsv[:, :, 1][color_mask]
                ))
            color_mask = self._template_cache[mask_key]
            template_median_saturation = self._template_cache[median_key]
        ...
        for (y, x) in zip(*locations):
            if color_mask is not None:
                ...
                if not np.any(color_mask):
                    continue
                patch_median_saturation = float(np.median(
                    patch_saturation[color_mask]
                ))
                if (
                    min_color_saturation is not None
                    and patch_median_saturation < min_color_saturation
                ):
                    continue
                if (
                    min_template_saturation_ratio is not None
                    and patch_median_saturation
                    < template_median_saturation * min_template_saturation_ratio
                ):
                    continue
    ...
    def sleep_interruptible(self, seconds, step=0.1):
        ...
        return self.running
    def report_cursor_error(self, error):
        '''限频报告鼠标定位失败，避免锁屏期间重复刷屏。'''
        now = time.time()
        last_error_at = getattr(self, '_last_cursor_error_at', 0)
        if now - last_error_at >= 5:
            print(
                f'鼠标定位失败: {error}；'
                '可能处于锁屏、远程桌面断开或窗口已移出屏幕，稍后重试'
            )
            self._last_cursor_error_at = now
    ...
    def click(self, x, y, duration=0.2, human_like=True):
        ...
        try:
            ...
            self.report_cursor_error(error)
            return False
        if not self.running:
            return False
    ...
    def find_team_up(self, screenshot=None):
        """判断能否发现队伍页面"""
        return self.find_template("team-up.png", screenshot=screenshot)
    ...
    def find_recruitment(self):
        while True and self.running:
            ...
            if team_up:
                self.exit_normal_team_before_battle()
                return True
            if not team_up:
                ...
                try:
                    ...
                    for template_name, threshold in [
                        ...
                        huanqiu_positions.extend(self.find_all_templates(
                            ...
                            min_template_saturation_ratio=0.78,
    ...
    def find_in_huanqiu_team(self, screenshot=None, attempts=3):
        """是否在环球队伍"""
        title_roi = None
        if self.game_window:
    ...
    def find_normal_stage_team(self, screenshot=None):
        if not self.game_window:
            return False
        if not hasattr(self, '_template_cache'):
            self._template_cache = {}
        ...
        frame = screenshot if screenshot is not None else self.take_screenshot()
        if frame is None:
            return False
        ...
        for template_name in [
            ...
            for scale_percent in range(70, 141, 5):
                ...
                scaled_key = ('edge-scaled', template_name, scale_percent)
                if scaled_key not in self._template_cache:
                    self._template_cache[scaled_key] = (
                        edge_template if scale_percent == 100 else cv2.resize(
                            edge_template,
                            None,
                            fx=scale,
                            fy=scale,
                ...
                scaled = self._template_cache[scaled_key]
    ...
    def find_normal_battle_title(self):
        '''快速识别战斗标题中的“数字.文字”排版，不依赖 OCR。'''
        frame = self.take_screenshot()
        if frame is None:
            return False
        frame_height, frame_width = frame.shape[:2]
        title_frame = frame[
            int(frame_height * 0.059):int(frame_height * 0.123),
            int(frame_width * 0.18):int(frame_width * 0.82),
        ]
        hsv = cv2.cvtColor(title_frame, cv2.COLOR_BGR2HSV)
        bright_text = (
            (hsv[:, :, 2] >= 165) & (hsv[:, :, 1] <= 180)
        ).astype(np.uint8)
        _, _, stats, _ = cv2.connectedComponentsWithStats(bright_text)
        components = [
            tuple(map(int, stat))
            for stat in stats[1:]
            if stat[4] >= 5
        ...
        for dot_x, dot_y, dot_width, _, _ in dots:
            digits = [
                box for box in components
                if 3 <= box[2] <= 14
                and 14 <= box[3] <= 25
                and 8 <= box[1] <= 30
                and dot_x - 38 <= box[0] < dot_x
                and dot_x - 7 <= box[0] + box[2] <= dot_x + 1
            ]
            text = [
                box for box in components
                if box[2] >= 14
                and box[3] >= 16
                and 8 <= box[1] <= 30
                and dot_x + dot_width <= box[0] <= dot_x + 100
            ]
            if digits and len(text) >= 2:
                return True
        return False
    def classify_battle_type(self):
        '''融合入口状态和标题视觉证据，返回关卡类型及判断详情。'''
        visual_huanqiu = self.find_huanqiu_battle()
        visual_normal = (
            False if visual_huanqiu else self.find_normal_battle_title()
        )
        self.last_normal_title_format = visual_normal
        visual_score = getattr(self, 'last_huanqiu_battle_score', 0.0)
        huanqiu_score = 0
        normal_score = 0
        reasons = []
        ...
        if visual_huanqiu:
            huanqiu_score += 5
            reasons.append(f'寰球标题={visual_score:.2f}')
        elif visual_normal:
            normal_score += 5
            reasons.append('普通标题=数字.文字')
        elif visual_score <= 0.55:
            normal_score += 3
            reasons.append(f'未见寰球标题={visual_score:.2f}')
        else:
            reasons.append(f'寰球标题弱匹配={visual_score:.2f}')

        if self.expecting_huanqiu_battle:
            huanqiu_score += 2
            reasons.append('入口状态=寰球队伍')
        if self.exit_normal_stage:
            normal_score += 3
            reasons.append('入口状态=普通关卡')

        detail = f'寰球{huanqiu_score}:普通{normal_score}；' + '，'.join(reasons)
        if huanqiu_score >= 5 and huanqiu_score - normal_score >= 2:
            return 'huanqiu', detail
        if normal_score >= 3 and normal_score - huanqiu_score >= 2:
    ...
    def find_dont_battle_return(self, screenshot=None, attempts=3):
        """判断是否有返回按钮"""
        for attempt in range(attempts):
    ...
    def exit_normal_team_before_battle(self):
        """用同一帧快速识别普通队伍，优先于战斗检测退出。"""
        if self.mode != 0 or not self.exit_normal_stage_on_huanqiu:
            return False
        frame = self.take_screenshot()
        if frame is None or not self.find_team_up(screenshot=frame):
            return False
        if self.find_in_huanqiu_team(screenshot=frame, attempts=1):
            return False
        self.initial_skill_check_deadline = None
        self.battle_identify_not_before = None
        ...
        self.expecting_huanqiu_battle = False
        self.group_wait_started_at = None
        print('组队界面快速识别为普通关卡，立即退出队伍')
        if self.find_dont_battle_return(screenshot=frame, attempts=1):
            self.find_click_continue()
            self.exit_normal_stage = False
            self.sleep_interruptible(0.15)
        else:
            # 退出按钮可能正在渐入；下一轮立即重试，不再做慢速战斗检测。
            self.sleep_interruptible(0.03)
        return True
    ...
    def main_loop(self, iterations=None):
        ...
        while self.running:
            ...
            while True and self.running:
                ...
                # 队长可能随时开始。先用一张截图检查并退出普通关卡队伍，
                # 避免 find_battling() 的多模板检测延误退出。
                if self.exit_normal_team_before_battle():
                    continue
                ...
                if should_identify_huanqiu:
                    battle_type, detail = self.classify_battle_type()
                    if battle_type == 'huanqiu':
                        huanqiu_identify_failures = 0
                        print(
                            f'融合判断为寰球远征（{detail}），取消普通关卡退出'
                        )
                        self.exit_normal_stage = False
                        ...
                        required_confirmations = (
                            1 if self.last_normal_title_format
                            else 3 if self.exit_normal_stage
                            else 5
                        )
                        if huanqiu_identify_failures < required_confirmations:
                            print(f'普通关卡候选，等待连续确认（{detail}）')
                            self.sleep_interruptible(0.3)
                            continue
                    else:
                        huanqiu_identify_failures = 0
                        print(f'战斗类型证据不足，保持未知并继续观察（{detail}）')
                        self.sleep_interruptible(0.2)
                        continue
            ...
            if self.mode == 0:
                ...
                # 上的招募入口绕过退出队伍流程。
                team_frame = self.take_screenshot()
                team_up = self.find_team_up(screenshot=team_frame)
                in_huanqiu_team = self.find_in_huanqiu_team(
                    screenshot=team_frame,
                    attempts=1,
                )
                normal_stage_team = (
                    (team_up and not in_huanqiu_team)
                    or self.find_normal_stage_team(screenshot=team_frame)
                )
...
class GameBotGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("游戏机器人操作界面（智能识别实验版）")
